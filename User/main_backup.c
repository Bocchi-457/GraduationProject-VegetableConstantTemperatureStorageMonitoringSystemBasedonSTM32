 ...
 * DHT22相关变量
 */
u8 data_valid = 0;                 // 温湿度数据有效标志（旧代码使用）
// volatile uint32_t sys_tick_ms = 0; // 全局毫秒计数器，已在 Timer.c 中定义
...
uint8_t Is_AT_Response(const char *buf) {
  ...
  if (strstr(buf, "connected") != NULL || strstr(buf, "CONNECT") != NULL)
    return 1;
  if (strstr(buf, "WIFI CONNECTED") != NULL ||
...
int main(void) {
  ...
  while (1) {
    ...
    if (calendar.hour == 3 && calendar.min == 0 &&
        calendar.w_date != last_sync_date && Flagout == 1) {
      need_time_sync = 1;
      time_sync_state = 0;
    }
    ...
    if (time_sync_state == 1 && local_len >= 19) {
      ...
      if (time_str_ptr != NULL) {
        ...
        if (sscanf(time_str_ptr, "%d-%d-%d %d:%d:%d", &r_year, &r_month, &r_day,
                   &r_hour, &r_minute, &r_second) == 6) {
          if (r_year >= 2000 && r_year <= 2099 && r_month >= 1 &&
              r_month <= 12 && r_day >= 1 && r_day <= 31 && r_hour >= 0 &&
              r_hour < 24 && r_minute >= 0 && r_minute < 60 && r_second >= 0 &&
              r_second < 60) {
      ...
      if (Flagout == 1) {
        ...
        if (need_time_sync) {
          ...
          sprintf(data,
                  ...
                  BEMFA_ID, mode, set_wendu_high, set_wendu_low, set_shidu_high,
                  set_shidu_low, jiare, zhileng, chushi, jiashi, sign_str,
    ...
    if (temp_diff >= 2.0f || humi_diff >= 5.0f) {
      ...
      beep_count++;
      if (beep_count >=
          5) // 大约5个主循环周期（根据实际主循环执行时间调整）触发一次蜂鸣状态切换
      {
        beep_count = 0;
    ...
    if (!Is_Bemfa_Data((const char *)local_buf)) {
      ...
      // 配置保存（延时写 Flash）
      if (config_changed &&
          (sys_tick_ms - last_save_time > CONFIG_SAVE_DELAY)) {
        SystemConfig_Save();
        config_changed = 0;
        last_save_time = sys_tick_ms;
...
u8 WeekYearday(int years, int months, int days) {
  ...
  // 使用基姆拉尔森计算公式计算星期几
  WeekDay = (days + 1 + 2 * months + 3 * (months + 1) / 5 + years + years / 4 -
             years / 100 + years / 400) %
            7;
...
void show_wendu(void) {
  ...
  int abs_temp = (DHT22_Data.temperature < 0) ? -DHT22_Data.temperature
                                              : DHT22_Data.temperature;
  char temp_sign =
      (DHT22_Data.temperature < 0) ? '-' : ' '; // 负数显示减号，正数补空格对齐
...
void SystemConfig_Save(void) {
  ...
  // 写入配置数据
  STMFLASH_Write(CONFIG_FLASH_ADDR, (uint16_t *)&system_config,
                 sizeof(SystemConfig_t) / 2);
}